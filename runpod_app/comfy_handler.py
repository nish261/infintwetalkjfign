...
def save_base64_to_file(base64_data, temp_dir, output_filename):
    ...
    try:
        # The API examples use data URLs. Python's base64 decoder expects only
        # the payload, so strip the MIME prefix when present.
        if isinstance(base64_data, str) and "," in base64_data and base64_data[:128].startswith("data:"):
...
def handler(job):
    ...
    if sampler_node_id:
        ...
        if "sample_steps" in job_input:
            inputs["steps"] = int(job_input["sample_steps"])
            logger.info(f"✅ 노드 {sampler_node_id} (WanVideoSampler) steps 업데이트됨: {inputs['steps']}")
